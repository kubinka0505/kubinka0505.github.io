...
class url {
	static main = {
		main: "https://kubinka0505.github.io"
	};
	...
	static games = {
		saves: "https://mega.nz/folder/zU9S0T6a#JhJHWefRIl9qBnICiTPDUg"
	};
...
class AudioPlayer {
	...
	static async avatar(dirname, max, avatarName) {
		const audioPath = `${Audio_Path}/${dirname}/`;
		const avatar = document.querySelector(".avatar");
		const avatarDirectory = "assets/media/img/png/Avatars/";
	
		const fileNumber = String(Random.integer(1, max)).padStart(3, "0");
		const file = `${fileNumber}.mp3`;
	
		avatar.src = `${avatarDirectory}${avatarName}.png`;
	
		try {
			await this.play(`${audioPath}${file}`, 0.25);
		} finally {
			avatar.src = `${avatarDirectory}Default.png`;
		}
	}
}
